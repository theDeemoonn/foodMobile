...
class UsersStore {
    ...
    async fetchMe() {
        ...
        try {
            ...
            this.setError(error instanceof Error ? error.message : 'Произошла неизвестная ошибка');
        } finally {
            this.setLoading(false);
        }
    }
    ...
    async addPaymentMethod(userId: string, paymentMethod: PaymentMethod) {
        this.setLoading(true);
        try {
            const response = await fetch(`https://api.example.com/users/${userId}/payment-methods`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async addToFavorites(userId: string, itemId: string) {
        this.setLoading(true);
        try {
            const response = await fetch(`https://api.example.com/users/${userId}/favorites`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async removeFromFavorites(userId: string, itemId: string) {
        this.setLoading(true);
        try {
            const response = await fetch(`https://api.example.com/users/${userId}/favorites/${itemId}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
